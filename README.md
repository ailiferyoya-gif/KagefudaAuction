...
仮想端末には、ドラッグ・最大化・最小化できるウィンドウ、Google風のローカル検索、サイト内ブラウザ、LINE風トーク、架空内線キーパッド、ローカル音声通話、エクスプローラー、メモ帳、ごみ箱を実装しています。検索候補・履歴・結果一覧を含む状態は同一ブラウザの `localStorage` にだけ保存されます。
