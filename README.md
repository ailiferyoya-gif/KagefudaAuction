...
仮想端末には、ドラッグ・最大化・最小化できるウィンドウ、Google風のローカル検索、サイト内ブラウザ、LINE風トーク、ローカルMail、Twitter風ローカル記録、架空内線キーパッド、ローカル音声通話、エクスプローラー、メモ帳、ごみ箱を実装しています。さらに音声レコーダー、カレンダー、写真、地図、ダウンロード、監視カメラ、付箋、設定を、それぞれ独立したデスクトップアプリとして直接起動できます。状態は同一ブラウザの `localStorage` にだけ保存されます。

実在の商品、団体、取引、人物、電話番号とは関係ありません。外部へのメッセージ送信、実電話への発信、マイク取得、決済、会員登録は発生しません。

Twitter風アプリでは、ホーム、検索、通知、ブックマーク、プロフィール、投稿、返信、いいね、リポストを操作できます。投稿は端末内にだけ保存され、実在するTwitter/Xや外部APIには接続しません。
