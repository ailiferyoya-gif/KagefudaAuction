...
- 入口: `tools.html`。`desktop.html` の「謎解きツール」アイコン、スタートメニュー、Google風ローカル検索から開く。URLは `https://ailiferyoya-gif.github.io/KagefudaAuction/tools.html`。
- 実装: `tools.html` / `tools.css` / `tools.js`。アプリを別ページに分離し、事件データは主に `tools.js` 冒頭の配列・辞書で差し替え可能にした。
